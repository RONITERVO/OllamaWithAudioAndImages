...
**Phase 3: Frontend Web Page (`templates/index.html`)**

1.  **Create `templates` Folder:** In the same directory as your Python script (`your_script_name.py`), create a folder named `templates`.
2.  **Create `index.html`:** Inside the `templates` folder, create the following `index.html` file:
...
<head>
    ...
    <style>
        ...
        .assistant {
            ...
            margin-right: auto; /* Align assistant messages to left */
            text-align: left;
        ...
        .error {
            ...
            font-weight: bold;
            text-align: center;
            margin: 0.5rem auto; /* Center errors */
            max-width: 90%;
        }
        .status {
             font-style: italic;
...
1.  **Save:** Save both the Python script (e.g., `chat_app_web.py`) and the `index.html` file in the `templates` subfolder.
2.  **Firewall:** Ensure your computer's firewall allows incoming connections on port `5000` (or the port you chose) from your local network. How to do this depends on your operating system (Windows Firewall, macOS Firewall, `ufw` on Linux).
3.  **Find Your PC's IP Address:**
    *   **Windows:** Open Command Prompt and type `ipconfig`. Look for the "IPv4 Address" under your active network connection (Wi-Fi or Ethernet).
    *   **macOS:** Open Terminal and type `ipconfig getifaddr en0` (for Wi-Fi, `en1` for Ethernet might also work) or `ifconfig | grep "inet "`.
    *   **Linux:** Open Terminal and type `ip addr show` or `ifconfig`. Look for the `inet` address associated with your active network interface (e.g., `eth0`, `wlan0`). It will likely be something like `192.168.1.x` or `10.0.0.x`.
4.  **Run the Python Script:**
    ```bash
    python your_script_name.py
    ```
    You should see output indicating both the Tkinter app starting and the Flask server starting:
    ```
    [Main] Starting Tkinter main loop...
    [Web] Starting Flask server on http://0.0.0.0:5000
    [Web] Access the UI from another device on your network via: http://<YOUR_PC_IP>:5000
    * Running on http://0.0.0.0:5000
    * Press CTRL+C to quit
    ```
    The Tkinter GUI should also appear.
5.  **Access Web UI:**
    *   On the *same computer* running the script, open a web browser and go to `http://127.0.0.1:5000` or `http://localhost:5000`.
    *   On *another device* (like your phone or another computer) connected to the *same local network* (e.g., same Wi-Fi), open a web browser and go to `http://<YOUR_PC_IP_ADDRESS>:5000` (replace `<YOUR_PC_IP_ADDRESS>` with the IP you found in step 3).
6.  **Test:**
    *   **Load:** Does the web page load correctly?
    *   **History:** Does the initial chat history appear (if any exists)?
    *   **Web Send:** Send a message from the web UI.
        *   Does it appear in the web UI chatbox ("You: ...")?
        *   Does a "Thinking..." or typing indicator appear?
        *   Does the message also appear in the *Tkinter* GUI's input box and then get processed? (Check console output too).
        *   Does the Ollama response stream back correctly into the *web UI* chatbox?
        *   Does the response also appear in the *Tkinter* chat history?
        *   Is the web input re-enabled after the response?
    *   **Tkinter Send:** Send a message using the Tkinter GUI.
        *   Does it process correctly in Tkinter?
        *   Does the history update correctly if you *refresh* the web UI page? (Real-time updates *from* Tkinter *to* the web UI for non-Ollama messages aren't implemented in this basic version).
    *   **SSE Connection:** Check the browser's developer console (usually F12) for any SSE connection errors or messages.
    *   **Concurrency:** Interact with both the Tkinter GUI and the web UI simultaneously. Do they interfere badly? (Some minor delays might occur).
    *   **Shutdown:** Close the Tkinter window. Check the console output to see if threads shut down reasonably cleanly and the Flask server stops.

This comprehensive plan integrates Flask, sets up the necessary communication, provides a functional frontend, and outlines testing steps. Remember that VAD/TTS via the web remains a much more complex challenge involving browser audio APIs (Web Audio API, getUserMedia) and potentially WebSockets or WebRTC for real-time audio streaming.